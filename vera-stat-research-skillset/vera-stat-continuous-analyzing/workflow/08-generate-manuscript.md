...
[Para 1: Distribution assessment — what was checked and why]
[Para 2: Group comparison methods — which tests, why chosen]
[Para 3: Subgroup analysis methods — if applicable]
[Para 4: Regression — model specification, predictors, why OLS]
[Para 5: Quantile regression — why, which quantiles, SE method]
...
- Vary whether CI level and α are stated explicitly or implied
...
- **Order A (hypothesis-driven):** Group comparison → Subgroup → Regression → QR → Trees → Comparison
- **Order B (model-driven):** Regression → Group comparison → Subgroup → QR → Trees → Comparison
- **Order C (exploratory-driven):** Distribution → Trees → Regression → QR → Group comparison → Subgroup → Comparison
