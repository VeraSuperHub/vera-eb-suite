...
- Select based on data context (variable names, direction, IRR magnitude)
...
- **Order A (hypothesis-driven):** Group comparison → Subgroup → Count models → Trees
- **Order B (model-driven):** Count models → Group comparison → Subgroup → Trees
- **Order C (exploratory-driven):** Distribution → Trees → Count models → Groups

### Table and figure naming:
Vary descriptive names: "Table 1. Poisson Regression Coefficients" vs
"Table 1. Predictors of [Outcome] (Incidence Rate Ratios)" vs
"Table 1. Count Model Results for [Outcome]"
...
- Practical significance framing ("clinically meaningful rate difference")
- Comparison to published benchmarks ("consistent with prior incidence studies")
- Limitation acknowledgment inline ("though the sample may be underpowered for zero-inflation detection")
- Methodological justification ("Negative Binomial preferred given overdispersion ratio of X.XX")
- Distributional insight ("the excess zeros may reflect a separate zero-generating process")
...
- Correct count model selection adapting to overdispersion and zero-inflation
- Automatic detection of event counts vs event rates (offset handling)
- Cross-method comparison with nuanced, context-specific interpretation
- Distributional assumption framing (not horse-race model selection)
- Discipline-specific reporting conventions for IRR, rates, and count data
