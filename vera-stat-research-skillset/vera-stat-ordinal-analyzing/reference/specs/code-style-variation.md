...
| Pattern | Prop. Odds | Multinomial | Random Forest | CART |
|---|---|---|---|---|
| A | `model_polr` | `model_multi` | `model_rf` | `model_cart` |
| B | `fit_ordinal` | `fit_multinomial` | `fit_forest` | `fit_tree` |
| C | `polr_result` | `multi_result` | `rf_result` | `cart_result` |
| D | `mod_polr` | `mod_multi` | `mod_rf` | `mod_cart` |
| E | `ord_fit` | `mnom_fit` | `rforest` | `dtree` |

Tidy output follows the same pattern:
- A: `polr_tidy`, `polr_glance`
- B: `fit_tidy`, `fit_summary`
- C: `polr_coefs`, `polr_stats`
- D: `mod_coefs`, `mod_fit`
- E: `ord_coefs`, `ord_summary`
...
# Models (any order): VGAM, nnet, rpart, randomForest
# Tests (any order): vcd, brant, DescTools, dunn.test
