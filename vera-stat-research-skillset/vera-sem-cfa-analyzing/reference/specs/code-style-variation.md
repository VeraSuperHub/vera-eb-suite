...
| Pattern | OLS | Quantile | Random Forest | LightGBM |
|---|---|---|---|---|
| A | `model_ols` | `model_qr` | `model_rf` | `model_lgbm` |
| B | `fit_linear` | `fit_quantile` | `fit_forest` | `fit_lgbm` |
| C | `lm_result` | `qr_result` | `rf_result` | `lgbm_result` |
| D | `mod_ols` | `mod_qr50` | `mod_rf` | `mod_lgbm` |
| E | `ols_fit` | `qreg_fit` | `rforest` | `lgbm_fit` |

Tidy output follows the same pattern:
- A: `ols_tidy`, `ols_glance`
- B: `fit_tidy`, `fit_summary`
- C: `lm_coefs`, `lm_stats`
- D: `mod_coefs`, `mod_fit`
- E: `ols_coefs`, `ols_summary`
...
```r
# Core (any order): tidyverse, broom, moments
# Models (any order): quantreg, rpart, randomForest, lightgbm
