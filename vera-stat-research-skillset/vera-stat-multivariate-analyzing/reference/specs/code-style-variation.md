...
| Pattern | MANOVA | Discriminant | PCA | CCA | LightGBM |
|---|---|---|---|---|---|
| A | `model_manova` | `model_lda` | `model_pca` | `model_cca` | `model_lgbm` |
| B | `fit_manova` | `fit_lda` | `fit_pca` | `fit_cca` | `fit_lgbm` |
| C | `manova_result` | `lda_result` | `pca_result` | `cca_result` | `lgbm_result` |
| D | `mod_manova` | `mod_lda` | `mod_pca` | `mod_cca` | `mod_lgbm` |
| E | `mv_fit` | `disc_fit` | `pc_fit` | `cc_fit` | `lgbm_fit` |

Tidy output follows the same pattern:
- A: `manova_summary`, `lda_scores`
- B: `fit_summary`, `fit_scores`
- C: `manova_stats`, `lda_coefs`
- D: `mod_summary`, `mod_scores`
- E: `mv_stats`, `disc_coefs`
...
```r
# Core (any order): tidyverse, broom, MVN
# Multivariate (any order): MASS, heplots, CCA, candisc
# Models (any order): randomForest, lightgbm, car
# Viz (already in tidyverse): GGally, gridExtra if needed
```
